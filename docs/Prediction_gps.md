# WLMap Feature Branch: prediction_gps

## Description
This branch shows a new circle on the map representing the predicted location from the Dloc model, updated continuously.

## Key Changes
This feature introduces the following changes:
-Receiving a new topic called "predicted_location" that contains the predicted GPS message
-Consistently display a green dot at the predicted location
...
- Consistently update and display the predicted user location
- Show the user’s location using the phone’s GPS
...
7. Once running the application, open LogCat and observe the following:
    ...
    - The predicted GPS location will be printed to the server log when received.
...
- [x] Prediction GPS data is receive from the MQTT server
